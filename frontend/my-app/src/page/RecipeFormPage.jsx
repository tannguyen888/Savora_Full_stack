...
export default function RecipeFormPage() {
  ...
  const { data: recipe, isLoading } = useQuery({
    queryKey: ["recipe", id],
    queryFn: async () => {
      const recipes = await savouraClient.entities.Recipe.filter({ id });
      return recipes[0];
    },
    enabled: isEdit && !!id,
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data) => savouraClient.entities.Recipe.create(data),
    onSuccess: (created) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => savouraClient.entities.Recipe.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["recipes"] });
